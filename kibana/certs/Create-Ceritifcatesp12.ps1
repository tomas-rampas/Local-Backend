...
& openssl x509 -req -in elasticsearch.csr -CA ca.crt -CAkey ca.key -passin "pass:$CaKeyPass" -CAcreateserial -out elasticsearch.crt -days 365
...
& openssl x509 -req -in kibana.csr -CA ca.crt -CAkey ca.key -passin "pass:$CaKeyPass" -CAcreateserial -out kibana.crt -days 365
